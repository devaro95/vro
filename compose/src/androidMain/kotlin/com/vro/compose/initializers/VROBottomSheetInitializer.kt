...
fun <VM : VRODialogViewModel<S, E>, S : VROState, E : VROEvent> InitializeViewModelBottomSheet(
    ...
    ModalBottomSheet(
        ...
        val stepper = viewModel.stepper.collectAsStateWithLifecycle(
            initialValue = content.skeleton?.let { VROSkeletonStep(viewModel.initialState) } ?: VROStateStep(viewModel.initialState),
            lifecycle = screenLifecycle
        ).value
        if (stepper is VROSkeletonStep) content.ComposableSkeleton()
        else {
            content.CreateDialog(state = stepper.state, events = viewModel, listener = listener, onDismiss = onDismiss)
            (stepper as? VRODialogStep)?.let { content.onDialog(it.dialogState) }
            (stepper as? VROErrorStep)?.let { content.onError(it.error, it.data) }
        }
    }
}
