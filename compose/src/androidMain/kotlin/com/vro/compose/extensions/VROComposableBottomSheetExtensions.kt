...
fun <VM : VROViewModel<S, D, E>, S : VROState, E : VROEvent, D : VRODestination> NavGraphBuilder.vroBottomSheet(
    ...
    bottomSheet(route = content.destinationRoute()) {
...
internal fun <VM : VROViewModel<S, D, E>, S : VROState, E : VROEvent, D : VRODestination> VroComposableNavBottomSheetContent(
    ...
    InitializeLifecycleObserver(screenLifecycle = screenLifecycle, viewModel = viewModel, navController = navigator.navController)
    val stepper = viewModel.stepper.collectAsStateWithLifecycle(
        initialValue = contentInstance.skeleton?.let { VROSkeletonStep(viewModel.initialState) } ?: VROStateStep(viewModel.initialState),
        lifecycle = screenLifecycle
    ).value
    if (stepper is VROSkeletonStep) contentInstance.ComposableDialogSkeleton()
    else {
        contentInstance.CreateDialog(stepper.state, viewModel, listener, onDismiss)
        (stepper as? VRODialogStep)?.let { contentInstance.onDialog(it.dialogState) }
        (stepper as? VROErrorStep)?.let { contentInstance.onError(it.error, it.data) }
    }
    InitializeEventsListener(viewModel)
    InitializeNavigatorListener(viewModel = viewModel, navigator = navigator)
}
